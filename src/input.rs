...
impl<'src, T> sealed::Sealed<T> for &'src T {}
...
pub trait Input<'src>: 'src {
    ...
    unsafe fn span(cache: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Span;
...
pub trait SliceInput<'src>: ExactSizeInput<'src> {
    ...
    fn full_slice(cache: &Self::Cache) -> Self::Slice;

    /// Get a slice from a start and end cursor
    // TODO: Make unsafe
    unsafe fn slice(cache: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Slice;

    /// Get a slice from a start cursor to the end of the input
    // TODO: Make unsafe
    unsafe fn slice_from(cache: &Self::Cache, from: RangeFrom<&Self::Cursor>) -> Self::Slice;
}
...
pub trait ValueInput<'src>: Input<'src> {
    ...
    /// # Safety
    ///
    /// `cursor` must be generated by either `Input::start` or a previous call to this function.
...
pub trait BorrowInput<'src>: Input<'src> {
    ...
    unsafe fn next_ref(cache: &Self::Cache, cursor: &mut Self::Cursor)
...
impl<'src> Input<'src> for &'src str {
    ...
    #[inline(always)]
    unsafe fn span(_this: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Span {
...
impl<'src> ExactSizeInput<'src> for &'src str {
    #[inline(always)]
    unsafe fn span_from(this: &Self::Cache, range: RangeFrom<&Self::Cursor>) -> Self::Span {
...
impl<'src> ValueInput<'src> for &'src str {
    #[inline(always)]
    unsafe fn next(this: &Self::Cache, cursor: &mut Self::Cursor) -> Option<Self::Token> {
        Self::next_maybe(this, cursor)
    }
}

impl<'src> Sealed for &'src str {}
...
impl<'src> SliceInput<'src> for &'src str {
    ...
    #[inline(always)]
    fn full_slice(this: &Self::Cache) -> Self::Slice {
        *this
    }

    #[inline(always)]
    unsafe fn slice(this: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Slice {
        &this[*range.start..*range.end]
    }

    #[inline(always)]
    unsafe fn slice_from(this: &Self::Cache, from: RangeFrom<&Self::Cursor>) -> Self::Slice {
        &this[*from.start..]
    }
}
...
impl<'src, T> BorrowInput<'src> for &'src [T] {
    #[inline(always)]
    unsafe fn next_ref(this: &Self::Cache, cursor: &mut Self::Cursor) -> Option<&'src Self::Token> {
        Self::next_maybe(this, cursor)
    }
}
...
impl<'src, const N: usize> Sealed for &'src [u8; N] {}
    ...
    S: Span + Clone + 'src,
{
    ...
    #[inline]
    unsafe fn span((cache, eoi): &Self::Cache, range: Range<&Self::Cursor>) -> Self::Span {
...
{
    #[inline(always)]
    unsafe fn span_from((cache, eoi): &Self::Cache, range: RangeFrom<&Self::Cursor>) -> Self::Span {
        let start = I::next_maybe(cache, &mut range.start.0.clone())
            .map(|tok| tok.borrow().1.start())
            // .or_else(|| self.input.next_maybe(self.input.start()).1)
            .unwrap_or_else(|| eoi.start());
...
{
    #[inline(always)]
    unsafe fn next((cache, _): &Self::Cache, cursor: &mut Self::Cursor) -> Option<Self::Token> {
    ...
    S: Span + Clone + 'src,
{
    ...
    #[inline(always)]
    fn full_slice((cache, _): &Self::Cache) -> Self::Slice {
        I::full_slice(cache)
    }

    #[inline(always)]
    unsafe fn slice((cache, _): &Self::Cache, range: Range<&Self::Cursor>) -> Self::Slice {
        I::slice(cache, &range.start.0..&range.end.0)
    }

    #[inline(always)]
    unsafe fn slice_from((cache, _): &Self::Cache, from: RangeFrom<&Self::Cursor>) -> Self::Slice {
    ...
    S::Offset: From<<I::Span as Span>::Offset>,
{
    ...
    #[inline]
    unsafe fn span((cache, ctx): &Self::Cache, range: Range<&Self::Cursor>) -> Self::Span {
...
{
    #[inline]
    unsafe fn span_from((cache, ctx): &Self::Cache, range: RangeFrom<&Self::Cursor>) -> Self::Span {
    ...
    F: Fn(I::Span) -> S,
{
    ...
    unsafe fn next_maybe(
        ...
        I::next_maybe(&cache.0, cursor)
    }

    #[inline]
    unsafe fn span(cache: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Span {
        let inner_span = I::span(&cache.0, range);
        (cache.1)(inner_span)
    }
}
    ...
    F: Fn(I::Span) -> S,
{
    #[inline(always)]
    unsafe fn span_from(cache: &Self::Cache, range: RangeFrom<&Self::Cursor>) -> Self::Span {
        let inner_span = I::span_from(&cache.0, range);
        (cache.1)(inner_span)
    }
}
    ...
    F: Fn(I::Span) -> S,
{
    ...
    #[inline(always)]
    fn full_slice(cache: &Self::Cache) -> Self::Slice {
        I::full_slice(&cache.0)
    }

    #[inline(always)]
    unsafe fn slice(cache: &Self::Cache, range: Range<&Self::Cursor>) -> Self::Slice {
        I::slice(&cache.0, range)
    }

    #[inline(always)]
    unsafe fn slice_from(cache: &Self::Cache, from: RangeFrom<&Self::Cursor>) -> Self::Slice {
        I::slice_from(&cache.0, from)
    }
}
...
impl<'src, R: Read + Seek + 'src> ValueInput<'src> for IoInput<R> {
    unsafe fn next(cache: &Self::Cache, cursor: &mut Self::Cursor) -> Option<Self::Token> {
        ...
        if *cursor != inner.last_cursor {
            let seek = *cursor as i64 - inner.last_cursor as i64;
...
impl<'src, 'parse, I: Input<'src>, E: ParserExtra<'src, I>> InputRef<'src, 'parse, I, E> {
        ...
        I: Input<'src>,
    {
        loop {
            ...
            let token = unsafe { I::next_maybe(&mut self.cache, &mut cursor) };
    ...
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
        let token = unsafe { I::next(&mut self.cache, &mut self.cursor) };
        if let Some(t) = &token {
    ...
    pub(crate) fn next_maybe_inner(&mut self) -> Option<I::TokenMaybe> {
        // SAFETY: cursor was generated by previous call to `Input::next`
        let token = unsafe { I::next_maybe(&mut self.cache, &mut self.cursor) };
        if let Some(t) = &token {
            self.state.on_token(t.borrow());
        }
        token
    ...
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
        let token = unsafe { I::next_ref(&mut self.cache, &mut self.cursor) };
        if let Some(t) = &token {
            self.state.on_token(t);
        }
        token
    ...
    #[inline(always)]
    pub fn peek_maybe(&self) -> Option<MaybeRef<'src, I::Token>> {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    /// Peek the next token in the input. Returns `None` if the end of the input has been reached.
    #[inline(always)]
    pub fn peek_ref(&self) -> Option<&'src I::Token>
    where
        I: BorrowInput<'src>,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    #[inline]
    pub fn slice(&self, range: Range<&Cursor<'src, 'parse, I>>) -> I::Slice
    where
        I: SliceInput<'src>,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    /// Get a slice of the input that covers the given cursor range.
    #[inline]
    pub fn slice_from(&self, range: RangeFrom<&Cursor<'src, 'parse, I>>) -> I::Slice
    where
        I: SliceInput<'src>,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    /// Get a slice of the input that covers the given cursor range.
    #[inline]
    pub fn slice_since(&self, range: RangeFrom<&Cursor<'src, 'parse, I>>) -> I::Slice
    where
        I: SliceInput<'src>,
    {
        // SAFETY: cursor was generated by previous call to `Input::next`
    ...
    #[inline(always)]
    pub fn span_from(&self, range: RangeFrom<&Cursor<'src, 'parse, I>>) -> I::Span
    where
        I: ExactSizeInput<'src>,
    {
        // SAFETY: `Cursor` is invariant over 'parse, so we know that this cursor came from the same input
    ...
    #[inline(always)]
    pub fn span_since(&self, before: &Cursor<'src, 'parse, I>) -> I::Span {
        // SAFETY: `Cursor` is invariant over 'parse, so we know that this cursor came from the same input
        // See `https://plv.mpi-sws.org/rustbelt/ghostcell/`
