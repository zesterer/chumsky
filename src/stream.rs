...
impl<Iter: Iterator> Stream<Iter::Item, Range<usize>> for IterStream<Iter> {
    ...
    fn last_span(&self) -> Option<Range<usize>> {
        self.1.checked_sub(1).map(|s| s..s + 1)
    }
...
impl<'a, I: Clone> IntoStream<I, Range<usize>> for &'a [I] {
    ...
    fn into_stream(self) -> Self::Stream {
        IterStream::new(self.iter().cloned())
    }
}
...
impl<I, const N: usize> IntoStream<I, Range<usize>> for [I; N] {
    ...
    fn into_stream(self) -> Self::Stream {
        IterStream::new(std::array::IntoIter::new(self))
    }
}
...
impl<Iter: Iterator<Item = (I, S)>, I, S: Span + Clone> Stream<I, S>
    for SpannedIterStream<Iter, S>
{
    fn peek(&mut self) -> Option<(&I, S)> {
        self.0.peek().map(|(x, span)| (x, span.clone()))
    }
...
impl<'a, I: Clone, S: Span + Clone> IntoStream<I, S> for &'a [(I, S)] {
    ...
    fn into_stream(self) -> Self::Stream {
        SpannedIterStream::new(self.iter().cloned())
    }
}
...
impl<I, S: Span + Clone> IntoStream<I, S> for Vec<(I, S)> {
    ...
    fn into_stream(self) -> Self::Stream {
        SpannedIterStream::new(self.into_iter())
    }
}
...
impl<I, S: Span + Clone, const N: usize> IntoStream<I, S> for [(I, S); N] {
    ...
    fn into_stream(self) -> Self::Stream {
        SpannedIterStream::new(std::array::IntoIter::new(self))
    }
}
