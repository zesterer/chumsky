    ...
    pub fn cbor<'a>() -> impl Parser<'a, &'a [u8], CborZero<'a>, extra::Err<Error<'a>>> {
        recursive(|data| {
            let read_int = any()
                ...
                .ignore_then_ctx(any().repeated().configure(|cfg, ctx| {
            ...
            let bstr = read_int.ignore_then_ctx(
                any()
                    .repeated()
            ...
            let str = read_int.ignore_then_ctx(
                any()
                    .repeated()
                    .configure(|cfg, ctx| cfg.exactly(*ctx as usize))
            ...
            let array = read_int.ignore_then_ctx(
                data.clone()
                    .with_ctx(())
                    .repeated()
                    .configure(|cfg, ctx| cfg.exactly(*ctx as usize))
            ...
            let map = read_int.ignore_then_ctx(
                data.clone()
                    .then(data.clone())
                    .with_ctx(())
                    .repeated()
